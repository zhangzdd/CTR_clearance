...
c = 30e-3;% 3mm clearance
max_clearance_step = 30;
...
if clearance_step ==1
    ...
    lambda_zzy = lambda_from_last_guess;
    stepSize = stepSize_from_last_guess;
end
...
for lp = 1:maxIter
    ...
    parfor m = 1:circle_sample
        for n = 1:Ni
         k = (m-1)*Ni+n;
         Sj = squeeze(S_zzy(1,n,:,:));
         Q_lambda = Q_lambda + lambda_zzy(k)*2*Jp'*(Sj'*Sj)*Jp;
        end
    end
    ...
    parfor m = 1:circle_sample
        for n = 1:Ni
         ...
         ci = c1{m};
         P2j = squeeze(P2(n,:,:));
         h_lambda = h_lambda + lambda_zzy(k)*2*((ci'*R1j'*P2j*Sj*Jp)'+(pstar'*(Sj'*Sj)*Jp)');
        end
    end
    ...
    for m = 1:circle_sample
       for n = 1:Ni
        ...
        ci = c1{m};
        P2j = squeeze(P2(n,:,:));
        a = -invQ*(2*Jp'*Sj'*Sj*Jp)*invQ*h_lambda + (-invQ)*2*((ci'*R1j'*P2j*Sj*Jp)'+(pstar'*(Sj'*Sj)*Jp)');
        %H = U + V + W + Z
        dU_dlambdak = delta_u_zzy'*2*(Jp'*Sj'*Sj*Jp)*delta_u_zzy + delta_u_zzy'*Q_lambda*a;
        dV_dlambdak = ci'*R1j*P2j*R1j*ci+2*ci'*R1j'*P2j*Sj*pstar+pstar'*Sj'*Sj*pstar;
        dW_dlambdak = 2*(ci'*R1j'*P2j*Sj*Jp+pstar'*Sj'*Sj*Jp)*delta_u_zzy + compute_coef_sum_W(c1,Rstar_1,P2,Jp,S_zzy,pstar,Ni,circle_sample,lambda_zzy)*a;
        dZ_dlambdak = -r2in^2;
        
        dlambda_zzy(k) = dU_dlambdak+dV_dlambdak+dW_dlambdak+dZ_dlambdak;                        
        end
    end
...
%         figure(3);
%         X = 1:1:Ni;
%         h = plot(X,dlambda_zzy + c);
%        
%     elseif lp>1
%         set(h,'XData',1:1:Ni,'YData',dlambda_zzy + c);
%         figure(3);
%     end
    disp(mean(dlambda_zzy));
    %q_col = reshape(q,(i-1)*Ni,1);
    J_lambda_zzy = 1/2*delta_u_zzy'*Q_lambda*delta_u_zzy+g'*delta_u_zzy+...
        compute_coef_sum_V(c1,Rstar_1,P2,Jp,S_zzy,pstar,Ni,circle_sample,lambda_zzy)+compute_coef_sum_W(c1,Rstar_1,P2,Jp,S_zzy,pstar,Ni,circle_sample,lambda_zzy)*delta_u_zzy-sum(lambda_zzy.*r2in^2);
...
%     
    ...
    save("pstar_ellip.mat","pstar_2_new","pstar_1_new");
